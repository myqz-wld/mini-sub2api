...
fn device() -> FingerprintSnapshot {
    FingerprintSnapshot::for_test(FingerprintMode::Device, 7, DEVICE_ID)
}
...
fn converges_all_recognized_carriers_and_preserves_other_identity() {
    ...
    let projected =
        project_http_device(headers, body, &device(), 1024 * 1024).expect("projected request");
    assert_eq!(
        projected
            .headers
...
fn valid_body_without_recognized_carrier_remains_byte_exact() {
    ...
    let projected = project_http_device(HeaderMap::new(), original.clone(), &device(), 1024)
...
fn preserves_turn_metadata_that_intentionally_omits_installation() {
    ...
    let projected = project_http_device(headers, body, &device(), 1024).expect("projected request");
...
fn rejects_unsafe_device_payloads_before_projection() {
    ...
    for (headers, body) in cases {
        assert!(project_http_device(headers, body, &device(), 1024).is_err());
    }
    ...
    assert!(project_http_device(encoded, Bytes::from_static(br#"{}"#), &device(), 1024).is_err());
}

#[test]
fn rejects_rewrite_that_exceeds_limit() {
    let body = Bytes::from_static(br#"{"client_metadata":{"x-codex-installation-id":"x"}}"#);
    assert!(project_http_device(HeaderMap::new(), body.clone(), &device(), body.len()).is_err());
}
...
fn websocket_projection_preserves_no_op_bytes_and_rewrites_carriers() {
    ...
    assert_eq!(
        project_websocket_device(no_op.clone(), &device(), 1024).expect("no-op frame"),
        no_op
    );
