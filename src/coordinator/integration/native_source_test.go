...
func nativeSource(t *testing.T) string {
	...
	if source == "" {
		source = filepath.Join(nativeRepository(), ".ref", "sources", "codex-v0.153.4")
	}
	...
	if err != nil || strings.TrimSpace(string(revision)) != "3d2ee51ca2d5db578f328aa75e20aa22c0197c9a" {
		t.Fatal("native parity requires the exact v0.153.4 source checkout")
	}
