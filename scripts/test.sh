...
if find src/coordinator -name '*.go' -print -quit 2>/dev/null | grep -q .; then
  ...
  NO_PROXY="127.0.0.1,::1" no_proxy="127.0.0.1,::1" \
    MINI_SUB2API_CORE_CODEX_BINARY="$repo_root/build/cargo-target/${CARGO_BUILD_TARGET:+$CARGO_BUILD_TARGET/}debug/mini-sub2api-core-codex" \
    mise exec -- go test -race -count=1 ./src/coordinator/...
fi
